 ...
 */
class PhoneTrainFragment : BaseFragment() {
...
//            startActivityForResult(NumberSelectFragment.obtainStartIntent(activity), REQUEST_SELECT_POKER)
    ...
    companion object {
        ...
        fun start(context: Context, numberCount:Int = 10) {
            val bundle = Bundle()
            bundle.putInt(KEY_REMEMBER_TYPE,numberCount)
            CommonFragmentActivity.start(context, PhoneTrainFragment::class.java.name, bundle)
